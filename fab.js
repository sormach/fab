function Fab(json){
  ...
  __element.insertAdjacentHTML('beforeend',
      `<button class='fab primary ${json.button.style}' id='${json.button.id}' data-toggle='tooltip' title='${json.button.tooltip}'>
      ${json.button.html}
  ...
  json.buttons.forEach(function(e, i){
    __element.querySelector('dl').insertAdjacentHTML('afterbegin',
        `<dt>
      <button id="${e.button.id}" class="fab ${e.button.style} btn${i}">
        ${e.button.html}
